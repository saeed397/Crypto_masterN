...
st.markdown(
    ...
    */
    ...
    /* ==============================================
    FIX: SIDEBAR COLLAPSE ARROW (RTL/LTR CONFLICT)
    ------------------------------------------------
    علت باگ:
    کل صفحه با "direction: rtl" چیده می‌شود، اما دکمه‌ی جمع/باز
    کردن سایدبار در خود Streamlit با فرض چیدمان LTR (Flexbox +
    transform) ساخته شده است. وقتی این المان‌ها داخل یک والد RTL
    قرار می‌گیرند، جهت flex آن‌ها (flex-start/flex-end) آینه
    می‌شود؛ در نتیجه دکمه هم در چپ و هم راست بالای صفحه ظاهر
    می‌شود و انیمیشن بسته‌شدن سایدبار با موقعیت واقعی دکمه
    هماهنگ نمی‌ماند (سایدبار نیمه‌باز وسط صفحه گیر می‌کند).

    راه‌حل:
    هدر بالای صفحه، خود سایدبار، و دکمه‌های کنترل آن را صراحتاً
    LTR نگه می‌داریم تا مستقل از جهت کلی صفحه، دقیقاً طبق منطق
    داخلی و پیش‌فرض Streamlit عمل کنند. این تغییر فقط ساختار
    داخلی این دکمه‌ها را اصلاح می‌کند و به متن/محتوای فارسی شما
    کاری ندارد.
    ============================================== */
